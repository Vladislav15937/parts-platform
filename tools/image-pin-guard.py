...
# Переехавший файл — одно сообщение на ВСЕ списки сторожа, и это не косметика
# (задачи 0181 и 0193). До 0181 расходились две половины: `_plant()` падал
# `FileNotFoundError`, а `check()` такой файл ТИХО ПРОПУСКАЛ. До 0193 ровно так
# же молчал перебор: его файлы объявлены своими списками, и `sweep()` пропускал
# ненайденный одной строкой `continue`, — а следом печатал «чужих — 11»
# и «Копий адреса нет», то есть УТВЕРЖДАЛ про то, чего не читал. Совсем слепы
# были четыре имени, которых в `LOOKED` нет вовсе: `db/docker-compose.yml`,
# оба workflow и `Dockerfile`, — то есть шаг прогона и рецепт боевого образа.
# Переименовали тест, рецепт зеркала или шаг прогона — ни копию адреса в нём,
# ни тянутые им образы не проверяет больше никто, а прогон зелёный. Именно
# в этом единственном случае копия и могла уехать незамеченной, так что молчать
# здесь нельзя ни одному списку.
GONE_MSG = "файл из списка не найден, сторож неполон"


class GuardIncomplete(RuntimeError):
    """Списки сторожа разошлись с деревом: собирать самопроверку не из чего."""


def declared_lists() -> tuple[tuple[str, tuple[str, ...]], ...]:
    """Списки, в которых файл ОБЪЯВЛЕН по имени, — вместе с именем списка.

    Списки остаются разными: они про разные вопросы (копия адреса против
    перебора образов), и склеить их значит однажды ответить не на тот.
    Одинаково у них одно — поведение на переехавшем файле.

    `SWEPT_GLOBS` и обход `src/test/java` сюда не входят: там файлы набираются
    образцом, и «объявленного, но пропавшего» там не бывает вовсе.
    """
    return (
        ("LOOKED", LOOKED),
        ("SWEPT_YAML", SWEPT_YAML),
        ("SWEPT_DOCKERFILES", SWEPT_DOCKERFILES),
    )


def declared_files() -> list[str]:
    """Объединение списков: каждый файл один раз, в порядке объявления."""
    names: list[str] = []
    for _, declared in declared_lists():
        for rel in declared:
            if rel not in names:
                names.append(rel)
    return names
...
def declared_missing(root: Path) -> dict[str, tuple[str, ...]]:
    """Объявленные файлы, которых в дереве нет: имя → списки, где он объявлен.

    Пусто — списки и дерево сходятся. Файл, объявленный дважды (оба compose
    и рецепты зеркала стоят и в `LOOKED`, и в списках перебора), назван один
    раз: сообщение про него одно, а списки в нём перечислены оба.
    """
    gone: dict[str, list[str]] = {}
    for label, declared in declared_lists():
        for rel in declared:
            if not (root / rel).is_file():
                gone.setdefault(rel, []).append(label)
    return {rel: tuple(labels) for rel, labels in gone.items()}
...
# Где перебор ищет. Списки свои, а не LOOKED: тот собран под другой вопрос
# (копии адреса зеркалённого образа), и склеивать два разных вопроса в один
# список значит однажды ответить не на тот. Ненайденный файл этих двух списков
# при этом — красное теми же словами, что у LOOKED (`GONE_MSG`): вопросы
# разные, поведение на переехавшем файле одно (задача 0193).
SWEPT_YAML = (
    "docker-compose.yml",
...
def sweep(root: Path) -> int:
    ...
    # Объявленные и не найденные — до обхода: по ним решается, можно ли вообще
    # что-то утверждать в конце. Красным о каждом говорит `check()`, одним
    # сообщением на все списки (`GONE_MSG`).
    swept_declared = SWEPT_YAML + SWEPT_DOCKERFILES
    gone = [rel for rel in swept_declared if not (root / rel).is_file()]
    for name in swept_files(root):
        path = root / name
        if not path.is_file():
            # Объявленный пропавший файл — красное ниже. Здесь остаётся защита
            # от гонки: файл, попавший в набор образцом (`SWEPT_GLOBS`, обход
            # тестов) и исчезнувший между обходом каталога и чтением.
            continue
        for ref in sorted(images_pulled_by(path)):
            ...
            bad = 1
    # Зелёное утверждение печатается только о прочитанном. Пока часть
    # объявленных файлов не найдена, «чужих — N» означало бы «столько нашлось
    # там, куда заглянули», а читается как «столько их в дереве»: ровно этим
    # сторож и врал до задачи 0193 — печатал «чужих — 11» и «Копий адреса нет»
    # при переименованном шаге прогона.
    if gone:
        red(f"  ✗ перебор образов не выполнен: не прочитано {len(gone)} "
            f"из {len(swept_declared)} объявленных файлов ({', '.join(gone)})")
        red("      Про каждый сказано выше отдельной строкой. Пока они не найдены,")
        red("      «чужих — N» было бы утверждением о том, чего сторож не читал.")
        return 1
...
def check(root: Path) -> int:
    ...
    # Сперва — сходятся ли списки сторожа с деревом. Молчаливый пропуск здесь
    # означает сторожа, ослепшего от переименования файла (см. GONE_MSG).
    for rel, lists in declared_missing(root).items():
        red(f"  ✗ {rel} — файла нет, а он в списке сторожа ({', '.join(lists)}): {GONE_MSG}")
        red("      Файл переехал или переименован — поправьте этот список в стороже,")
        red("      иначе ни копию адреса в нём, ни образы, которые он тянет,")
        red("      не проверяет больше никто.")
        bad = 1
...
def _plant(dst: Path) -> None:
    ...
    **Не `cp -R` корня репозитория, и это цена живого отказа.** Рядом с рабочим
    ...
    Файлы копируются по именам, поэтому время самопроверки больше не зависит
    от того, что лежит рядом. Берутся они из **всех** объявленных списков,
    а не из одного `LOOKED`: до задачи 0193 дерево самопроверки собиралось
    из него одного, и в нём не было ни одного файла перебора (`ci.yml`,
    `deploy.yml`, `Dockerfile`, `db/docker-compose.yml`). То есть на подделках
    перебор молчал не потому, что проверил, а потому, что читать было нечего,
    — и случай с переехавшим файлом перебора поставить было некуда.

    Сколько их — не написано здесь словом намеренно: написанное руками число
    разошлось со списком в первый же день (`tasks/0181`), поэтому его печатает
    самопроверка из `len(declared_files())`.
...
def selftest() -> int:
    ...
    # Число файлов печатается из самих списков, а не пишется словом: написанное
    ...
    if gone:
        ...
        red("      Дерево самопроверки собирают из этих списков — собирать не из чего.")
        return 1
    print(f"  ✓ списки сторожа полны: файлов в них {len(declared_files())}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Восьмой случай — переехавший файл ПЕРЕБОРА, и он про задачу 0193.
        # Перебор объявляет свои файлы своим списком, и четырёх из них в LOOKED
        # нет вовсе: на них сторож проходил ЗЕЛЁНЫМ, печатая «чужих — 11»
        # и «Копий адреса нет». Берём `ci.yml` именно потому, что в LOOKED его
        # нет: красное на нём может дать только перебор, и случай не подменяется
        # седьмым. Проверяется и то, что зелёного утверждения больше нет:
        # сторож, называющий число чужих образов там, где часть файлов
        # не прочитана, врёт ровно тем, что у него зелено.
        fake = Path(tmp) / "swept-file-moved"
        _plant(fake)
        moved = fake / ".github/workflows/ci.yml"
        if not moved.is_file():
            # Словами, а не traceback'ом: файла нет в дереве самопроверки —
            # значит `_plant()` собирает его не из всех списков, и случай
            # проверяет пустоту. Ровно это и было до задачи 0193.
            red(f"  ✗ подделку негде поставить: дерево самопроверки без {moved.name}")
            red("      `_plant()` обязан собирать дерево из всех объявленных списков.")
            return 1
        moved.rename(moved.parent / "ci.yml.moved")
        rc, out = probe(fake)
        if rc != 0 and GONE_MSG in out and "перебор образов: чужих" not in out:
            print("  ✓ переехавший файл перебора — красное, и числа чужих образов нет")
        else:
            red(f"  ✗ переехавший файл перебора обязан валить сторожа словами «{GONE_MSG}»")
            red("      и не печатать «перебор образов: чужих — N»: это утверждение")
            red("      о том, чего сторож не читал.")
            bad = 1
