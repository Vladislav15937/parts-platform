...
**Подсказка со свободными номерами не кончается пустотой.** Искала она их
в первой тысяче, а схем в тестах уже сотня и каждая волна добавляет: заняв
тысячу, сторож стал бы валить прогон со строкой «Возьмите свободный номер:»
и пустым местом под ней — то есть человек прочёл бы это как поломку самого
сторожа, а не как исчерпание номеров (`docs/defect-classes.md`, класс 4:
обрезанный список молчит об обрезке, и пустота на месте ответа читается
как «ничего нет»). Теперь диапазон — весь, какой позволяет имя схемы
(`t_` и шесть цифр, то есть по t_999999), а на случай, когда кончится
и он, стоит отказ словами. Обе половины проверяются самопроверкой:
занять первую тысячу — обязан предложить t_001000, занять весь диапазон
(в самопроверке он короткий) — обязан сказать, что случилось.
...
# Докуда идут номера схем. Предел не выдуман, а взят из самого имени: `t_`
# и шесть цифр (SCHEMA), то есть последняя схема — t_999999. Подсказка искала
# свободный номер в первой тысяче и на этом кончалась: схем в тестах уже сотня,
# каждая волна добавляет, а исчерпав тысячу, сторож стал бы отдавать пустую
# строку на месте совета — то есть выглядел бы сломавшимся, а не исчерпавшимся.
LIMIT = 10 ** 6
...
def free_numbers(mentioned, count=8, limit=LIMIT):
    ...
    Диапазон — весь, какой позволяет имя схемы, а не первая тысяча. `limit`
    вынесен параметром ради самопроверки: исчерпать диапазон она обязана
    за два десятка номеров, а не за миллион — миллион она бы считала заметное
    время, а идёт она перед каждым прогоном.
    """
    busy = {int(s[2:]) for s in mentioned}
    free = (f"t_{n:06d}" for n in range(1, limit) if n not in busy)
    return list(itertools.islice(free, count))


def free_advice(mentioned, count=8, limit=LIMIT):
    """Подсказка про свободные номера — номерами или отказом словами.

    Пустое место там, где человек ждёт номер, читается как «сторож сломался»,
    а не как «номера кончились» (`docs/defect-classes.md`, класс 4: обрезанный
    список, молчащий об обрезке). Кончиться диапазону теперь почти неоткуда,
    но ответ на этот случай стоит трёх строк, а молчание — круга разбирательств
    о том, что сломалось в самом стороже.
    """
    free = free_numbers(mentioned, count, limit)
    if free:
        return "Свободные номера: " + ", ".join(free) + "."
    return (f"Свободных номеров нет: заняты все от t_000001 до "
            f"t_{limit - 1:06d}. Шире диапазона нет — имя схемы это `t_` "
            f"и шесть цифр; освобождайте номера разведёнными схемами.")
...
def check(root=TESTS, allowed=ALLOWED, remote=None, extra_mentions=(), limit=LIMIT):
    owners, mentioned, unresolved = survey(root, remote)
    problems = []
    advice = free_advice(mentioned | set(extra_mentions), limit=limit)
    ...
    for schema in sorted(shared):
        ...
        problems.append(
            ...
            f"      порядке, где чужой класс идёт первым. Номер нужен такой, какого\n"
            f"      нет ни здесь, ни в origin/main:\n"
            f"        {advice}\n"
...
def selftest():
    ...
    try:
        ...
        # 6. Тысяча — не предел: имя схемы это `t_` и шесть цифр. Заняв первую
        #    тысячу целиком, человек обязан получить t_001000, а не пустоту.
        busy = {f"t_{n:06d}" for n in range(1, 1000)}
        problems, _, _ = check(root, allowed={}, extra_mentions=busy)
        if "t_001000" not in "\n".join(problems):
            failures.append("первая тысяча номеров занята, и подсказка молчит "
                            "о номерах за ней, хотя имя схемы — шесть цифр: "
                            "сторож объявит исчерпание на одной сотой диапазона")

        # 7. Диапазон исчерпан по-настоящему — сторож говорит словами, что
        #    случилось. Пустота на месте подсказки читается как поломка самого
        #    сторожа, а не как «номера кончились», и первым её так и прочтут.
        busy = {f"t_{n:06d}" for n in range(1, 21)}
        problems, _, _ = check(root, allowed={}, extra_mentions=busy, limit=21)
        if not problems:
            failures.append("общая схема перестала быть нарушением, когда "
                            "кончились свободные номера")
        elif "Свободных номеров нет" not in "\n".join(problems):
            failures.append("номера кончились, а сторож предлагает выбрать "
                            "из пустоты: совет без единого номера — это отказ, "
                            "притворившийся советом")
...
def main():
    ...
    if "--selftest" in sys.argv:
        print("Сторож краснеет на общей схеме и на номере, занятом в origin/main;\n"
              ...
              "подсказывает номера за первой тысячей, а кончившись — говорит\n"
