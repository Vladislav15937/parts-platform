...
Чего проверка не видит и почему этого хватает. Запись в `public.tenant_registry`
— второй общий ресурс: тест, чистящий чужую строку реестра, отключает соседу
вход. Здесь она не проверяется, потому что номер записи по соглашению равен
номеру схемы (`t_000144` — `tenant_id = 144`), а номер схемы стережётся. Пара
ExportRoleTest и OrganizationAuditTest делила ровно то и другое разом, и разошлись
они одним движением. Заводя схему, поставьте тот же номер и в реестр — иначе
столкновение переедет туда, куда сторож не смотрит.
