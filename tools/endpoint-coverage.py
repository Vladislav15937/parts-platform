...
# Два новых нашлись 9 сентября 2026 — не обходом, а тем, что проверка
# научилась отличать обёртку от экрана (задача 0055): до этого они считались
# вызванными и в отчётах выглядели работающими. Один из двух в тот же день
# и закрыт — просроченные резервы стали колонкой доски сделок (0052).
KNOWN = {
    ...
    "GET /api/deals/expired-reservations":
        "устарел: просроченные резервы человек видит колонкой «Истек срок» "
        "на доске сделок (GET /api/deals/board, задача 0052) — она собирает "
        "их тем же условием, и DealBoardTest сверяет два набора между собой. "
        "Обёртка expiredReservations из sales/sales.ts убрана как заменённая; "
        "сам путь оставлен сверочным",
