...
#
# Раздел «счётчик выгрузки против файла» однажды свалился разбором JSON
# (сессия кончилась, и вместо списка приехало сообщение об ошибке), напечатал
# traceback — и скрипт закончил словами «Всё сходится». То есть инструмент,
# которым отвечают на вопрос «работает ли система», зачёл упавшую проверку
# за успешную; а спрашивают его как раз тогда, когда подозревают неладное.
# Ловушка ловит любой отказ команды и любой выход по ошибке: раздел, который
# не смог посчитать, обязан быть виден так же ясно, как и несошедшийся.
trap 'bad "проверка оборвалась на строке ${LINENO} — раздел не посчитан"' ERR
set -E
