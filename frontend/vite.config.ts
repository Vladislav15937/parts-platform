...
export default defineConfig({
  ...
  test: {
    ...
    environment: 'jsdom',
    // По умолчанию vitest подменяет содержимое CSS пустой строкой — включая
    // импорт текстом (`app.css?raw`). Тест раскладки (`narrowViewport`) меряет
    // страницу настоящим браузером и без настоящих стилей мерил бы голую
    // разметку: он был зелёным и на сломанной вёрстке, проверено откатом.
    // Компонентов, импортирующих CSS, в тестах нет — его тянет только
    // `main.tsx`, — так что включение ни на что больше не влияет.
    css: true,
