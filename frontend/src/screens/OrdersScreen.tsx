...
export function OrdersScreen({ canSell }: { canSell: boolean }) {
  ...
  async function decline(deal: Deal) {
    ...
    setBusy(deal.id);
    try {
      // Причина уезжает в историю документа, и она разная: «нечем
      // обеспечить» и «покупатель отказался» — это не одно и то же,
      // а разбирают историю через недели.
          ...
          return (
            <li key={deal.id} className={unfulfilled ? 'card card--alert' : 'card'}>
              ...
              {/* Отклонить можно любой заказ, а не только необеспеченный.
                  Покупатель передумал, деталь при осмотре оказалась хуже
                  объявления, её не нашли на полке — по защищённой сделке
                  продавец обязан ответить, и если ответить нечем, он делает
                  это в кабинете площадки, а у нас заказ висит в очереди
                  вечно и держит резерв. Половину этой болезни уже лечили,
                  когда экран называл отказ и не давал его сделать. */}
              {canSell && (
                <button
                  type="button"
