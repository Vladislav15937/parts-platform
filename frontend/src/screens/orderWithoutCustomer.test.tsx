 ...
 *
 * <p><b>Прежняя оговорка «обычной продаже клиент по-прежнему нужен» снята
 * задачей 0011.</b> Половина продаж на разборке — человек с улицы, и теперь
 * в поле клиента стоит контрагент «Частное лицо»; проверка этого живёт
 * в {@code retailCustomer.test.tsx}. Здесь осталось своё утверждение,
 * к нему не сводящееся: кнопка заказа с площадки оживает от **номера
 * заказа**, а не от клиента, — и оно верно независимо от того, подставлен
 * клиент или нет.
 */
describe('заказ с площадки', () => {
  ...
  it('кнопка оживает от номера заказа, а не от клиента', async () => {
    ...
    // Заказ с площадки: клиента у него нет вовсе, и оживает кнопка
    // от номера заказа.
