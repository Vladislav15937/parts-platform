...
/**
 * Кто скачивает склад одним файлом — витрину и вкладку колёс.
 *
 * <p>Сами экраны открыты всем вошедшим (`ANYONE`): продавцу нужна цена
 * и наличие, кладовщику — полка. Файл — другое: опись всего имущества
 * разборки одним нажатием, и уносит её любой, кто вошёл. Себестоимости
 * в нём нет, но есть вся номенклатура с ценами, поставками, заметками
 * и остатками по каждому складу.
 *
 * <p>Список — решение владельца продукта от 9 сентября 2026
 * (`tasks/0050-vygruzka-sklada-dostupna-vsem.md`), тот же, что у отчётов.
 * Тот же список стоит на сервере в `CatalogController.EXPORTS`, и он там
 * главный: спрятанная кнопка при открытом эндпоинте — это не защита.
 *
 * <p><b>Заплатка, названная заплаткой.</b> Задача 0044 меняет жёсткие роли
 * на полномочия; тогда это становится полномочием «скачивать таблицу»,
 * и список уходит отсюда вместе с серверным.
 */
