...
import { MONTHS } from '../ui/shortDate';
...
export interface FeedSettings {
  ...
  /**
   * Дописывать ли к описанию объявления стоимость установки детали.
   *
   * <p>`null` и `false` — не дописывать. Услуга у клиента заведена и стоит
   * денег, а покупатель о ней не знал вовсе: «Цена установки» есть
   * в карточке и в отборе выгрузки, а до объявления не доезжала ни одной
   * строкой.
   */
  installationNote: boolean | null;
  /** Текст приписки; `{цена}` в нём заменяется стоимостью установки. */
  installationTemplate: string | null;
}

/**
 * Куда в тексте приписки встаёт цена.
 *
 * <p>Повторяет серверную константу `FeedSettings.PRICE_PLACEHOLDER`: подстановку
 * набирает владелец руками, и разойдись эти два написания — приписка молча
 * уехала бы к покупателю без суммы.
 */
 ...
 *
 * <p>Поле открывается заполненным: включённая приписка с пустым текстом
 * была бы включённой возможностью, которая ничего не делает. Формулировка
 * та же, что у системы, с которой переходят клиенты.
 */
export const DEFAULT_INSTALLATION_TEMPLATE =
...
export interface Feed {
  ...
  /**
   * Когда выгрузку удалили. `null` — живая.
   *
   * <p>Удалённая не приходит в обычном списке вовсе: её нет ни в перечне,
   * ни в счётчиках. Приходит она только по отдельному запросу — ради ответа
   * на вопрос «эта выгрузка вообще работала и когда её последний раз
   * забирали»: строка остаётся именно поэтому.
   */
...
/**
 * Удалённые выгрузки — отдельным запросом, а не признаком в общем списке.
 *
 * <p>Удаление помечает строку, а не убирает её: удалить запись значило бы
 * унести историю отправок, а спрашивают именно её — «эта выгрузка вообще
 * работала и когда её последний раз забирали». Но в списке и в счётчиках
 * удалённой быть не должно, иначе «удалена» значит «помечена», и владелец
 * пересчитывает свои прайс-листы глазами.
 */
export function listDeletedFeeds(): Promise<Feed[]> {
  return request<Feed[]>('/api/marketplace-accounts?deleted=true');
}

/**
 * Переименовывает выгрузку.
 *
 * <p>Ссылку это не трогает: её прописывает в кабинете площадки техспециалист
 * руками, и связать её с названием значило бы превратить исправление опечатки
 * в остановку выгрузки на несколько дней.
 */
export function renameFeed(id: number, title: string): Promise<Feed> {
  return request<Feed>(`/api/marketplace-accounts/${id}/title`, {
    method: 'PUT',
    body: { title },
  });
}
 ...
 *
 * <p>Выключенная не отдаёт прайс вовсе — площадка получает «нет такой»
 * и попробует позже. Не пустой прайс: пустой файл она читает буквально,
 * «этих товаров больше нет», и снимает объявления вместе с накопленными
 * просмотрами.
 */
export function setFeedStatus(id: number, status: 'ACTIVE' | 'PAUSED'): Promise<Feed> {
  return request<Feed>(`/api/marketplace-accounts/${id}/status`, {
    method: 'PUT',
    body: { status },
  });
}
 ...
 * Удаляет выгрузку.
 *
 * <p>На сервере это пометка: ссылка перестаёт работать, из списка выгрузка
 * пропадает, название и имя файла освобождаются, а отметки о заборе прайса
 * и об отправках остаются — их и спрашивают, когда объявления пропали.
 */
export function deleteFeed(id: number): Promise<void> {
  return request<void>(`/api/marketplace-accounts/${id}`, { method: 'DELETE' });
}

/**
 * Состояние выгрузки словами.
 *
 * <p>«PAUSED» на экране не значит ничего, а разница видна площадке: у
 * выключенной прайс не забирают вовсе. Ошибку показываем отдельной строкой,
 * она про последнюю отправку, а не про состояние.
 */
export function statusLabel(feed: Feed): string {
  if (feed.status === 'PAUSED') {
    return 'выключена';
  }
  // ERROR пишет не человек, а неудачная отправка. Показать его «работает»
  // значило бы соврать про состояние, а «выключена» — соврать про причину.
  return feed.status === 'ERROR' ? 'ошибка' : 'работает';
}

/** Когда выгрузку удалили — тем же коротким видом, что и отметка о заборе. */
export function deletedMark(feed: Feed): string {
  return feed.deletedAt === null ? '' : `Удалена ${moment(feed.deletedAt)}`;
}
...
export interface FeedSettingsInput {
  ...
  /** Переключатель уезжает как есть: у «да/нет» пустого состояния нет. */
  installationNote: boolean;
  /** Пусто — «текст не задавали», и сервер возьмёт формулировку по умолчанию. */
  installationTemplate: string | null;
}
...
/*
 * Таблица месяцев общая (`ui/shortDate`), а не своя на экран: короткое имя
 * в `ru-RU` зависит от сборки ICU («сент.» против «сен»), поэтому `Intl` тут
 * не годится, — а три копии таблицы уже разъехались падежом. Число месяца
 * не годится вовсе: «04.09» рядом со временем читается как ещё одно время.
 */
