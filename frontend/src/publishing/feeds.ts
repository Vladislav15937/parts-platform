...
export interface FeedSettings {
  ...
  /**
   * Выгружать ли товар, по которому ожидается поступление: он привязан
   * к поставке в пути и на складе его ещё нет.
   *
   * <p>`null` и `false` — не выгружать. Это единственная настройка, которая
   * меняет не только вид объявления, но и состав прайса, — поэтому её
   * состояние уходит и в счётчик позиций.
   */
  expectedGoods: boolean | null;
  /**
   * Приписка, которая встаёт **в начало** описания такого объявления.
   *
   * <p>Умолчания у неё нет: пустой текст при включённом переключателе
   * сервер отбивает словами. Товар, уехавший без объяснения, — это
   * объявление, по которому покупатель приедет за деталью, лежащей
   * в контейнере.
   */
  expectedGoodsNote: string | null;
}
...
export function countMatching(
  ...
  //
  // Переключатель ожидаемого товара едет рядом с отбором, хотя живёт
  // в настройках сборки: состав прайса он меняет, и счётчик, о нём
  // не знающий, обещал бы меньше, чем уедет площадке. Как и весь остальной
  // отбор, он берётся из формы — до сохранения.
  return request<{ parts: number }>('/api/marketplace-accounts/filter/count', {
    method: 'POST',
    body: { ...filter, productLine, expectedGoods },
  });
}
...
export interface FeedSettingsInput {
  ...
  installationTemplate: string | null;
  /** Переключатель «выгружать товары, по которым ожидается поступление». */
  expectedGoods: boolean;
  /** Пусто при включённом переключателе — отказ словами: умолчания тут нет. */
  expectedGoodsNote: string | null;
}
