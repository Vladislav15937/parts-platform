...
public class SalesService {
     ...
     *
     * <p><b>Поиск — четыре ветки через {@code UNION}, а не четыре условия
     * через {@code OR}.</b> С {@code OR} планировщик берёт индекс, только если
     ...
     * {@code part_code_trgm}, номер позиции уникальным {@code part_number_uk}.
     * Тот же приём и по той же причине, что в поиске продавца и на витрине
     * склада.
     *
     * <p><b>Номер позиции — четвёртая ветка (задача 0069), и она про тот же
     * звонок, ради которого завели третью.</b> Разговор на разборке начинается
     * не с публичного кода — того у звонящего нет, он неугадываемый и живёт
     ...
     *
     * <p><b>Номер сделки и номер позиции сравниваются точно, код детали
     * и клиент — вхождением.</b> Номер человек называет целиком («восьмая»,
     * «триста сорок седьмая»), и подстрока от него находит чужие документы —
     * 1347 и 3470; имя клиента и код детали называют по памяти и по кускам.
     *
     * <p>Цифровой запрос при этом идёт <b>обеими</b> числовыми ветками разом:
     * «347» отдаёт и сделку №347, и сделки с позицией 347. Разделять их
     * пришлось бы вопросом «что вы имели в виду» — а продавец на линии
     * набирает то, что ему продиктовали, не зная, номер это документа или
     * товара. То же решение, что в поиске склада (задача 0064): выдача
     * не подменяется, к ней добавляется.
     *
     ...
     * @param query    поиск: точное совпадение по номеру сделки и по номеру
     *                 позиции, вхождение — по имени клиента и по публичному
     *                 коду детали
