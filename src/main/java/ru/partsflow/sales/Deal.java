...
public class Deal {
     ...
     *
     * <p>Не {@link #reserve}: тот поднимает статус до {@code RESERVED}
     * и годится для черновика, а продление — это разговор про уже отложенный
     * товар. Выданная и отменённая срока не имеют вовсе, и продлевать у них
     * нечего: отказ говорит это словами, а не молча пишет дату в закрытый
     * документ.
     *
     * <p>Просроченный резерв продлевается — ради этого продление и нужно:
    ...
    public void extendReservation(Instant until) {
        if (status != DealStatus.RESERVED) {
            throw new IllegalStateException(
                    "Срок резерва продлевают у отложенной сделки, а эта в состоянии " + status);
        }
        if (until == null || !until.isAfter(Instant.now())) {
            throw new IllegalArgumentException("Срок резерва должен быть в будущем");
        }
    ...
    /**
     * Меняет контрагента: розничная продажа оказалась не розничной.
     *
     * <p>Порядок на разборке — сначала товар, потом клиент: сделка
     * открывается на «Частном лице», а имя покупателя выясняется по ходу
     * разговора. Поэтому смена контрагента — обычный шаг, а не исправление
     * ошибки, и она отдельным путём, а не сеттером: у поля два условия,
     * и оба про деньги.
     *
     * <p><b>Закрытый документ не трогаем.</b> У отменённой и возвращённой
     * сделки контрагент — часть того, что уже случилось; переписав его,
     * мы сказали бы, что товар возвращал не тот, кто возвращал.
     *
     * <p><b>И не трогаем, если по сделке двигались деньги.</b> Платёж
     * записан на прежнего клиента, зачёт списан с его счёта, переплата
     * легла туда же — сменив контрагента, мы оставили бы деньги одного
     * человека в документе другого, и касса перестала бы сходиться
     * с лицевыми счетами. Выданная и неоплаченная сделка при этом
     * переписывается: долг обязан числиться за тем, кто увёз товар.
     */
    public void changeCustomer(Long customerId) {
        ...
        if (status.isClosed()) {
            throw new IllegalStateException(
                    "Сделка закрыта (%s): менять контрагента у закрытого документа нельзя"
                            .formatted(status));
        }
        if (paidAmount.signum() != 0) {
            throw new IllegalStateException(
                    "По сделке уже проходили деньги — смена контрагента оставила бы платежи "
                            + "за прежним клиентом. Оформите возврат или заведите новую сделку");
        }
        this.customerId = customerId;
    }
    ...
    /**
     * Сколько клиент должен по этой сделке.
     *
     * <p><b>У закрытой сделки долга нет, и это не косметика.</b> Разность
     * «сумма минус оплачено» у отменённой и возвращённой отвечает на вопрос,
     * которого никто не задавал: товара у клиента нет, платить не за что.
     * Продавец же видел «Долг 1000 ₽» у детали, которую клиент сам принёс
     * обратно, — и шёл звонить за деньгами. Рядом с этим числом стоит кнопка
     * «Зачесть со счёта», и нажатая, она забирала у клиента его же деньги
     * в счёт возвращённого товара.
     */
