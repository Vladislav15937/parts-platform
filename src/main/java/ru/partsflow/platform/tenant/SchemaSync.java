 ...
 *
 * <p><b>И вниз — не бесконечно.</b> У отката есть объявленная нижняя
 * граница ({@link TenantSchemaMigrator#rollbackFloor()}): ниже неё
 * одиннадцать выпущенных changeset'ов схему не возвращают, и молчаливый
 * «успех» там был бы хуже отказа — база объявила бы себя версией, которой
 * не является. Такая цель отбивается словами, до того как тронута хоть
 * одна схема. Довод и замеры — {@code db/CLAUDE.md}, задача 0102.
 *
...
public class SchemaSync {
    ...
    public Result run(Plan plan) {
        ...
        Problem tooLow = belowFloor(target);
        if (tooLow != null) {
            return new Result(null, List.of(), 0, List.of(tooLow));
        }
    ...
    /**
     * Отказ вместо отката ниже объявленной границы (задача 0102).
     *
     * <p>Ниже неё откат схему <b>не возвращает</b>: одиннадцать выпущенных
     * changeset'ов не отменяют того, что сделали, — и проходит это молча,
     * с кодом ноль. Получалась бы ровно та пара «код одной версии, база
     * другой», ради которой режим и написан, только хуже: база объявила бы
     * себя версией 100, не будучи ею. Поэтому здесь отказ словами, а не
     * молчаливый успех.
     *
     * <p><b>Отказ и в режиме проверки тоже.</b> «Сколько не хватает до версии
     * 100» — вопрос про цель, которой у этого инструмента нет; ответив на него
     * числом, он пообещал бы, что {@code --apply} туда приведёт.
     *
     * <p>Граница живёт в артефакте, а не в аргументе: {@code
     * db/changelog/rollback-floor.properties} едет вместе с changelog'ом,
     * и опускающий схему образ спрашивает свою.
     *
     * @return {@code null}, если цель достижима
     */
    private Problem belowFloor(int target) {
        String floor = migrator.rollbackFloor();
        int lowest = TenantSchemaMigrator.changeSetsIn(floor);
        if (target >= lowest) {
            return null;
        }
        // «Версия недостижима», а не «опустить нельзя»: цель ниже границы
        // отбивается и тогда, когда для отставшей схемы это был бы накат
        // вверх, — а отказ, называющий не то действие, читают как чужой.
        return new Problem(CELL, "Версия " + target
                + " недостижима: ниже объявленной границы отката " + floor
                + ". Ниже неё откат схему не возвращает — одиннадцать "
                + "выпущенных changeset'ов не отменяют того, что сделали "
                + "(волна «логика в Java», переименование ключей, два снятых "
                + "NOT NULL), и схема получилась бы не той, на которой та "
                + "сборка работала. Самая младшая достижимая версия — "
                + lowest + "; за ней — возврат парного слепка, а не миграция "
                + "(db/CLAUDE.md, «Нижняя граница отката объявлена»)");
    }
