...
public class TenantSchemaMigrator {
    ...
    /**
     * Версия схемы — идентификатор последнего changeset'а в поставке.
     *
     * <p>Не порядковый номер и не дата: changeset'ы добавляются только в конец,
     * а их идентификаторы говорят человеку, чего именно не хватает клиенту.
     * Одинаково для всех арендаторов, поэтому считается один раз на прогон.
     */
    public String expectedVersion() {
        return inLiquibase(null, liquibase -> {
            List<ChangeSet> all = liquibase.getDatabaseChangeLog().getChangeSets();
            return all.isEmpty() ? null : all.get(all.size() - 1).getId();
        });
    }
