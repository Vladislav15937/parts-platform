 ...
 *
 * <p><b>Кто это вызывает.</b> Пока никто: слоя потребителей событий в проекте
 * нет, {@code deal.issued.v1} доезжает до транспорта и там заканчивается.
 * Метод {@link #onDealIssued} — готовая точка подключения, и вызвать его должен
 * потребитель, когда тот появится.
 */
@Service
