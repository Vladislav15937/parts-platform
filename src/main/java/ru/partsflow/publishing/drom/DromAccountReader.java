 ...
 *
 * <p><b>Выключенная и удалённая сюда не попадают, и в этом весь смысл
 * выключения.</b> Прайс-лист закрывают на сезон, а по живой ссылке площадка
 * продолжала бы забирать товар, которого клиент там видеть не хочет. Закрыто
 * это одним условием на оба чтения — выдачу прайса и отправку дельт: два
 * условия в двух местах разошлись бы, и выключенная выгрузка продолжала бы
 * получать дельты в прайс-лист, которого нет. Отсутствие прайса, а не пустой
 * прайс: пустой файл площадка читает буквально — «этих товаров больше нет» —
 * и снимает объявления вместе с накопленными просмотрами.
 */
@Component
