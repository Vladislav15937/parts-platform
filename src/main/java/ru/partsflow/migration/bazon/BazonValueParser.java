...
public final class BazonValueParser {
     ...
     *
     * <p><b>«Везде» — не «обычное написание», а то самое.</b> Так эта колонка
     * подписана в таблице товаров прежней системы, и наша витрина списана
     * с неё слово в слово: {@code CASE WHEN p.is_published THEN 'Везде'}.
     * То есть значение, которое вернее всего и придёт в выгрузке, читалось
     * как непонятное — и клиент, включивший колонку по инструкции, получал
     * весь склад без разрешения публиковать. Ровно та беда, о которой
     * предупреждает отчёт при отсутствии колонки, только молча:
     * предупреждение не срабатывает, потому что колонка на месте.
     */
    public static Boolean parsePublishFlag(String raw) {
        return switch (normalize(raw)) {
            case "да", "yes", "true", "1", "+", "выгружать", "везде" -> Boolean.TRUE;
