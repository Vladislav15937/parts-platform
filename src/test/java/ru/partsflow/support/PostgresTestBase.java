...
public abstract class PostgresTestBase {
     ...
     * а тесты создают {@code t_000001}…{@code t_000201} сами и в реестр
    ...
    /**
     * Заводит схемы арендаторов для одного тестового класса.
     *
     * <p>Схема принадлежит одному классу, и берут её отсюда: свободный номер
     * подскажет {@code ./tools/test-schema-guard.py}, он же валит прогон, если
     * номер уже занят соседом. Правило «схему на два теста не делить» нарушено
     * четырежды за три дня — каждый раз новым способом (остаток, имя владельца,
     * имя продавца, пароль) и каждый раз зелено локально: поодиночке класс
     * заводит фикстуру сам, сталкиваются они только в общем прогоне и только
     * в том порядке, где чужой класс идёт первым.
     *
     * <p>Тот же номер ставьте и в {@code public.tenant_registry}, если класс
     * туда пишет: сторож смотрит на схему, а чужую строку реестра тест удаляет
     * так же легко.
     */
