 ...
 * это законно, и проверка обязана про это сказать, а не промолчать. Зелёную
 * половину — «роль есть, журналы ей не принадлежат, склад правится» —
 * проверяет {@code JournalProtectionTest} настоящей рабочей ролью и
 * настоящими правами Postgres, но без HTTP-слоя: подменить пользователя
 * базы в контексте Spring нельзя, имя и пароль ставит
 * {@code @DynamicPropertySource} базового класса. Целиком по HTTP это
 * состояние подтверждено живым прогоном (PR #176), автотеста у него нет —
 * сказано прямо, чтобы ссылка на несуществующий класс не выглядела
 * покрытием.
 *
