 ...
 *
 * <p><b>Журналы здесь честно красные.</b> В этом контексте база одна и роль
 * одна — это законно, и проверка обязана про это сказать, а не промолчать.
 * Зелёную половину — «роль есть, журналы ей не принадлежат, склад
 * правится» — проверяет {@link AppReadinessHealthyCellTest}: две настоящие
 * роли Postgres и ответ 200 по HTTP (задача 0088). Отдельным контекстом,
 * потому что имя и пароль базы ставит {@code @DynamicPropertySource}
 * базового класса, а он старше свойств {@code @SpringBootTest}: роль
 * подставляется только подменой бинов источников данных.
 *
...
class AppReadinessTest extends PostgresTestBase {
    ...
    void schemaAheadStaysReady() throws Exception {
        ...
        // журналы честно красные. Зелёный ответ целиком — ниже,
        // cellWithoutTenantsIsReady.
        JsonNode body = readiness(503);
    ...
    void otherChecksAreGreen() throws Exception {
        ...
        assertThat(detail(body, "writes"))
                .as("про запись сказано не «проходит» при работающей базе: проба, "
                        + "отвечающая «заморожена» или «не проверена» на обычной базе, "
                        + "сделала бы слово «заморожена» в ответе бессмысленным — "
                        + "оно перестало бы отличать брошенную сборку от любой")
    ...
    void cellWithoutTenantsIsReady() throws Exception {
        ...
        // все четыре проверки разом: «готов» обязан быть достижимым, иначе
