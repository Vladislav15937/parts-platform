 ...
 * можно только из-под роли, которой журналы не принадлежат, а свойствами
 * её не подставить: имя и пароль базы ставит
 * {@code @DynamicPropertySource} базового класса, и он старше свойств
 * {@code @SpringBootTest}. Здесь хватает роли и настоящих прав — четыре
 * состояния проверяются без Spring вовсе. Тот же ответ по HTTP, вместе
 * с остальными четырьмя проверками готовности, отдаёт
 * {@code AppReadinessHealthyCellTest}: там ради этого поднят отдельный
 * контекст с подменёнными источниками данных (задача 0088).
 */
