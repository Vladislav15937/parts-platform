...
class DromFeedStreamTest extends PostgresTestBase {
    ...
    /**
     * <p><b>Первая отметка отодвигается назад, а не берётся от соседнего
     * забора.</b> Два запроса подряд укладываются в одну микросекунду, и
     * «строго позже» падает на ровном месте — CI покраснел так на правке,
     * которая кода не касалась вовсе. Шаткий тест хуже отсутствующего: он учит
     * перезапускать, а не читать, и однажды перезапустят настоящую поломку.
     *
     * <p>Час назад — заведомо больше любой разницы между двумя запросами,
     * и проверка от этого не слабеет: отметка, оставшаяся на месте, по-прежнему
     * роняет тест.
     */
    @Test
    ...
    void secondDownloadMovesTheMark() {
        get();
        inTenant(() -> jdbc.update("UPDATE marketplace_account "
                + "SET last_feed_download_at = now() - interval '1 hour' "
                + "WHERE title = ?", "Поток"));
        java.time.Instant moved = mark();
