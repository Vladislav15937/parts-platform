...
арендатора. В общей схеме `catalog` рискованных откатов ещё 28 (справочник
машин, виды деталей, реестр арендаторов), и цена у них другой природы:
это данные ячейки, а не клиента, и собираются они заново из `db/seed`.
Молчание тут читалось бы как «там терять нечего» — поэтому: там тоже есть
что терять, просто список ещё не написан.
"""
import argparse
import os
import re
